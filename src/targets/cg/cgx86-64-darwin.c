...
char regs[][13] = {"%rdi", "%rsi", "%rcx", "%rdx", "%r8",  "%r9",  "%rax",
                   "%r10", "%r11", "%rbx", "%r12", "%r13", "%r14", "%r15"};

char regs_32[][13] = {"%edi",  "%esi",  "%ecx",  "%edx",  "%r8d",
                      "%r9d",  "%eax",  "%r10d", "%r11d", "%ebx",
                      "%r12d", "%r13d", "%r14d", "%r15d"};

char regs_byte[][13] = {"%dil",  "%sil",  "%cl",   "%dl",   "%r8b",
                        "%r9b",  "%al",   "%r10b", "%r11b", "%bl",
                        "%r12b", "%r13b", "%r14b", "%r15b"};

char FPURegs[][7] = {"%xmm0", "%xmm1", "%xmm2", "%xmm3",
                     "%xmm4", "%xmm5", "%xmm6", "%xmm7"};

void gc_data() { sg_operator(DATA_LABEL); }

void gc_text() { sg_operator(TEXT_LABEL); }

void gc_prelude(void) {}
void gc_postlude(void) {}

int gc_stack_align(int size) {
...
void gc_def_byte(int v) { define_data(BYTE_LABEL, v); }

void gc_def_long(long int v) { define_data(LONG_LABEL, v); }

void gc_def_quad(int v) { define_data(QUAD_LABEL, v); }
...
void gc_call(char *function) { sg_unary_operation(CALL, function); }

void gc_pushFPU(int reg) {
  ng_binary_operation(SUB, 16, rSP);
  sg_binary_operation(MOVDQU, FPURegs[reg], gc_reference(rSP));
}

void gc_popFPU(int reg) {
...
void gc_setcc(char *op, int reg) { sg_unary_operation(op, regs_byte[reg]); }
...
long int gc_jmp(long int lab) {
  char adr[112];
  sprintf(adr, "%c%-19ld", LPREFIX, lab);
  sg_unary_operation(JMP, adr);
  return ftell(Outfile) - 20;
}
...
void gc_public(char *label) { sg_unary_operation(GLOBL_LABEL, label); }
