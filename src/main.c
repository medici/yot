...
char*
newfilename(char *file, int sfx) {
    char* ofile;

    if ((ofile = strdup(file)) == NULL)
    	mark("too many file names", NULL);
    if (sfx != 0) {
